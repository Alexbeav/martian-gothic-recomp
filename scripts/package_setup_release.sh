...
#
# Writes: dist/martian-gothic-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix martian-gothic \
  --exe-name Martian_Gothic_Recompiled \
  --display-name "Martian Gothic Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Martian Gothic - Unification (Europe) (En,Fr,Es,It).cue" \
